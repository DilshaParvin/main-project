...
def login_view(request):
    if request.method == 'POST':
        ...
        if user:
            ...
            # Redirect superusers and staff to Django admin
...
def feed_view(request):
    ...
    # Promoted recipes and active promotions stay as-is
    promoted_recipes = Recipe.objects.filter(is_promoted=True).order_by('-updated_at')[:5]
    promotions = SpecialOffer.objects.filter(is_active=True).order_by('-start_date')[:5]
